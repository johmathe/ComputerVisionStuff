...
#include "detect_regions.h"

void DetectRegions::setFilename(string s) { filename = s; }
...
vector<Plate> DetectRegions::segment(Mat input) {
  ...
  if (showSteps) imshow("Threshold", img_threshold);
  ...
  if (showSteps) imshow("Contours", result);
