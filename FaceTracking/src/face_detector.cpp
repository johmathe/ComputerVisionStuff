...
#define fl at<float>
//==============================================================================
vector<Point2f> face_detector::detect(const Mat &im, const float scaleFactor,
  ...
  detector.detectMultiScale(
      eqIm, faces, scaleFactor, minNeighbours,
      0 | CV_HAAR_FIND_BIGGEST_OBJECT | CV_HAAR_SCALE_IMAGE, minSize);
  if (faces.size() < 1) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    p[i].y =
        scale[2] * reference.fl(2 * i + 1) + R.y + 0.5 * R.height + scale[1];
  }
  return p;
}
//==============================================================================
void face_detector::train(ft_data &data, const string fname, const Mat &ref,
                          const bool mirror, const bool visi, const float frac,
                          const float scaleFactor, const int minNeighbours,
                          const Size minSize) {
  ...
  for (int i = 0; i < data.n_images(); i++) {
    ...
    if (faces.size() >= 1) {
      ...
      if (this->enough_bounded_points(pt, faces[0], frac)) {
        ...
        yoffset.push_back((center.y - (faces[0].y + 0.5 * faces[0].height)) /
                          w);
    ...
    if (mirror) {
      ...
      if (faces.size() >= 1) {
        ...
        if (this->enough_bounded_points(pt, faces[0], frac)) {
          Point2f center = this->center_of_mass(pt);
          float w = faces[0].width;
          xoffset.push_back((center.x - (faces[0].x + 0.5 * faces[0].width)) /
                            w);
          yoffset.push_back((center.y - (faces[0].y + 0.5 * faces[0].height)) /
                            w);
          zoffset.push_back(this->calc_scale(pt) / w);
...
//==============================================================================
bool face_detector::enough_bounded_points(const Mat &pts, const Rect R,
                                          const float frac) {
...
void face_detector::write(FileStorage &fs) const {
  assert(fs.isOpened());
  fs << "{"
     << "fname" << detector_fname << "x offset" << detector_offset[0]
     << "y offset" << detector_offset[1] << "z offset" << detector_offset[2]
     << "reference" << reference << "}";
}
//==============================================================================
