...
//==============================================================================
void ft_data::draw_points(Mat& im, const int idx, const bool flipped,
                          const Scalar color, const vector<int>& pts) {
  if ((idx < 0) || (idx >= (int)imnames.size())) return;
  int n = points[idx].size();
  if (pts.size() == 0) {
    for (int i = 0; i < n; i++) {
      if (!flipped)
        circle(im, points[idx][i], 1, color, 2, CV_AA);
      else {
...
//==============================================================================
void ft_data::draw_sym(Mat& im, const int idx, const bool flipped,
  ...
  for (int i = 0; i < n; i++) {
    if (colors[i] == Scalar::all(0.0)) {
      colors[i] =
          Scalar(rn.uniform(0, 255), rn.uniform(0, 255), rn.uniform(0, 255));
...
//==============================================================================
void ft_data::draw_connect(Mat& im, const int idx, const bool flipped,
                           const Scalar color, const vector<int>& con) {
  if ((idx < 0) || (idx >= (int)imnames.size())) return;
  ...
  if (con.size() == 0) {
    for (int i = 0; i < n; i++) {
      ...
      if (!flipped)
        line(im, points[idx][j], points[idx][k], color, 1);
      else {
    ...
    for (int j = 0; j < m; j++) {
      ...
      if (!flipped)
        line(im, points[idx][k], points[idx][l], color, 1);
      else {
...
void ft_data::write(FileStorage& fs) const {
  ...
  for (int i = 0; i < int(connections.size()); i++) {
    char str[256];
    const char* ss;
    sprintf(str, "connections %d 0", i);
    ss = str;
    fs << ss << connections[i][0];
    sprintf(str, "connections %d 1", i);
    ss = str;
    fs << ss << connections[i][1];
  }
  fs << "n_symmetry" << (int)symmetry.size();
  for (int i = 0; i < int(symmetry.size()); i++) {
    char str[256];
    const char* ss;
    sprintf(str, "symmetry %d", i);
    ss = str;
    fs << ss << symmetry[i];
  }
...
void ft_data::read(const FileNode& node) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    sprintf(str, "connections %d 0", i);
    ss = str;
    node[ss] >> connections[i][0];
    sprintf(str, "connections %d 1", i);
    ss = str;
    node[ss] >> connections[i][1];
  }
  node["n_symmetry"] >> n;
  symmetry.resize(n);
  for (int i = 0; i < n; i++) {
