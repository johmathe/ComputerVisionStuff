...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < pmodel.n_patches(); i++) {
    Mat I1;
    normalize(pmodel.patches[i].P, I1, 0, 255, NORM_MINMAX);
    Mat I2;
  ...
  while (1) {
    ...
    P[i].copyTo(I);
    rectangle(img, points[i],
              Point(points[i].x + P[i].cols, points[i].y + P[i].rows),
    ...
    if (i < 0)
      i = 0;
    else if (i >= pmodel.n_patches())
      i = pmodel.n_patches() - 1;
  }
