...
void face_tracker_params::write(FileStorage& fs) const {
  ...
  for (int i = 0; i < int(ssize.size()); i++) {
    char str[256];
    const char* ss;
    sprintf(str, "w %d", i);
    ss = str;
    ...
    sprintf(str, "h %d", i);
    ss = str;
    fs << ss << ssize[i].height;
  }
  fs << "robust" << robust << "itol" << itol << "ftol" << ftol << "scaleFactor"
     << scaleFactor << "minNeighbours" << minNeighbours << "minWidth"
...
void face_tracker_params::read(const FileNode& node) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    sprintf(str, "w %d", i);
    ss = str;
    node[ss] >> ssize[i].width;
    sprintf(str, "h %d", i);
    ss = str;
    node[ss] >> ssize[i].height;
  }
  node["robust"] >> robust;
  node["itol"] >> itol;
  node["ftol"] >> ftol;
...
//==============================================================================
void read(const FileNode& node, face_tracker_params& x,
...
//==============================================================================
void face_tracker::draw(Mat& im, const Scalar pts_color,
...
//==============================================================================
vector<Point2f> face_tracker::fit(const Mat& image, const vector<Point2f>& init,
                                  const Size ssize, const bool robust,
                                  const int itol, const float ftol) {
  int n = smodel.npts();
  cout << n << endl;
  cout << smodel.npts() << endl;
  ...
  if (!robust) {
    ...
    for (int iter = 0; iter < itol; iter++) {
      ...
      float v = 0;
      for (int i = 0; i < n; i++) v += norm(pts[i] - pts_old[i]);
      if (v < ftol)
        break;
      else
        pts_old = pts;
...
void face_tracker::write(FileStorage& fs) const {
  assert(fs.isOpened());
  fs << "{"
     << "detector" << detector << "smodel" << smodel << "pmodel" << pmodel
     << "}";
}
//==============================================================================
